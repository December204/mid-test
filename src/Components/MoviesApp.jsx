...
import weather from '../assets/img/id1.jpg';
import oncePeace from '../assets/img/id2.jpg';
import boruto from '../assets/img/id3.jpg';
import spy from '../assets/img/id4.jpg';
import shingeki from '../assets/img/id5.png';
import captain from '../assets/img/id6.jpg';
import ao from '../assets/img/id7.jpg';
const MoviesApp = () => {
  const animeList = [
    {
      "id": "1",
  ...
  const displayAnimes = animeList.filter(anime => anime.id !== selectedAnime.id);

  return (
    <div style={{backgroundColor:'#192026',color:'#868686',padding:'15px 83px',gap:'29px',font:'Poppins'}}>
      <div className="flex justify-between items-center mb-8">
        <nav className="flex gap-60 text-lg">
          <h1 className="text-3xl font-bold text-white">Anonime</h1>
          <a href="#" >Home</a>
          <a href="#" >List anime</a>
          <input
            type="text"
            placeholder="Search anime or movie"
            style={{backgroundColor:"#374151",color:'#868686',borderRadius:'37px',marginTop:'5px',padding:'8px 100px 8px 18px'}}
          />
        </nav>
      </div>
      ...
      <section className="mb-10">
        ...
        <div className="rounded-xl overflow-hidden relative h-auto bg-gradient-to-r">
          <img
            src={selectedAnime.image}
            alt="Explore Banner"
            className="w-[1500px] h-[450px] object-cover"
          />
          <div className="absolute bottom-4 left-6 text-white w-2/3">
            <h3 className="text-2xl font-bold">{selectedAnime.movieName}</h3>
      ...
      <section>
        <h2 className="text-2xl font-bold mb-4 text-white">New Release</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {displayAnimes.map((anime) => (
            <div 
              key={anime.id} 
              className=" rounded-xl overflow-hidden cursor-pointer transform transition-all duration-200 hover:scale-105 "
            ...
            >
              ...
              />
              <div className="p-2">
                <p className="text-sm text-white">Episode {anime.episode}</p>
                <p className="text-white font-semibold text-sm">{anime.movieName}</p>
              </div>
            </div>
          ))}
        </div>
